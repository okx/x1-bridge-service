...
func start(ctx *cli.Context) error {
	...
	bridgeService := server.NewBridgeService(c.BridgeServer, c.BridgeController.Height, networkIDs, apiStorage, []*utils.Client{nil}, []*bind.TransactOpts{nil})
	...
	for i, l2EthermanClient := range l2Ethermans {
		...
		go runSynchronizer(ctx.Context, 0, bridgeController, l2EthermanClient, c.Synchronizer, storage, zkEVMClient, chExitRootEventL2, nil, chSyncedL2, []uint{}, nil, nil)
	}
	chSynced := make(chan uint)
	go runSynchronizer(ctx.Context, c.NetworkConfig.GenBlockNumber, bridgeController, l1Etherman, c.Synchronizer, storage, nil, nil, chsExitRootEvent, chSynced, networkIDs, nil, nil)
	go func() {
		for {
			select {
			case netID := <-chSynced:
...
func runSynchronizer(ctx context.Context, genBlockNumber uint64, brdigeCtrl *bridgectrl.BridgeController, etherman *etherman.Client, cfg synchronizer.Config, storage db.Storage, zkEVMClient *client.Client, chExitRootEventL2 chan *etherman.GlobalExitRoot, chsExitRootEvent []chan *etherman.GlobalExitRoot, chSynced chan uint, allNetworkIDs []uint, messagePushProducer messagepush.KafkaProducer, redisStorage redisstorage.RedisStorage) {
	sy, err := synchronizer.NewSynchronizer(ctx, storage, brdigeCtrl, etherman, zkEVMClient, genBlockNumber, chExitRootEventL2, chsExitRootEvent, chSynced, cfg, allNetworkIDs, messagePushProducer, redisStorage)
	if err != nil {
		log.Fatal(err)
	}
